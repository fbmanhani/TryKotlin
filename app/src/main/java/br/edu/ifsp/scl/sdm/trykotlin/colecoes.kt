...
fun main() {
    // List e MutableList
    val familia: List<String> = listOf("Pedro", "Marcela", "João", "Cadu")
    ...
    // Percorrer usando FOR usando IntRange
    for (i in 0 until listaInteiros2.size) {
...
*/
    ...
    // Solução 3
    val funcao = fun (ci: IndexedValue<String>) {
        println("${ci.index} - ${ci.value}")
    }

    val g: (IndexedValue<String>) -> Unit
    g = funcao
    ...
    val familiaMap: MutableMap<String, String> = mutableMapOf(Pair("Pai","Pedro"),Pair("Mãe","Marcela"),Pair("Filho","João"),Pair("Caçula","Cadu"),Pair("Pet","Paçoca"))
